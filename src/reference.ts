...
import type { Conditions, SegmentFn, Utils } from "./_utils";
...
export class Reference {
    ...
    encounterTable = (
        engine: EngineAPI,
        tagFn: SegmentFn = this.utils().lastSegment,
    ) => {
        const scope = this.currentScope();
        const pathRegex = this.utils().segmentFilterRegex(scope);
        const data: Encounter[] = this.utils()
            .filesWithPath(pathRegex)
            .map((tfile: TFile) => {
        ...
        markdownBuilder.createTable(
            ["Name", "Where", "Affiliation"],
            data
                .filter((x) => x.status === "active")
                .map((x) => [x.link, x.where, x.affiliation]),
        );

        markdownBuilder.createHeading(2, "New");
        markdownBuilder.createTable(
            ["Level", "Name", "Where", "Affiliation"],
            data
                .filter((x) => x.status === "new")
                .map((x) => [x.level, x.link, x.where, x.affiliation]),
        );

        markdownBuilder.createHeading(2, "Other");
        markdownBuilder.createTable(
            ["Status", "Name", "Where", "Affiliation"],
            data
                .filter((x) => !x.status.match(/(active|new)/))
                .map((x) => [x.status, x.link, x.where, x.affiliation]),
        );
    ...
    factionTable = (
        ...
        for (const tfile of this.app.vault.getMarkdownFiles()) {
            if (tfile === this.app.workspace.getActiveFile()) {
                // exclude the file containing the table
                continue;
            }
            if (pathPattern && !this.utils().filterByPath(tfile, pathPattern)) {
                // skip the file if there is a pattern and the file doesn't match
                continue;
            }

            const fileTags = this.utils().fileTags(tfile);
            const groupTag = fileTags.find((t) => t?.startsWith("type/group"));
            if (groupTag) {
                const result = [
                    this.utils().markdownLink(tfile),
                    groupTag.substring(11),
                ];
                if (!pathPattern) {
    ...
    itemsForTagRaw = (tag: Tags, type: string, scoped = false): string[] => {
        ...
        const makeLink = (tfile: TFile, linkAlias = "") => {
            let name = linkAlias;
        ...
        for (const tfile of this.app.vault.getMarkdownFiles()) {
            ...
            const fileTags = this.utils().fileTags(tfile);
            const hasTag = tagRegexes.some((regex) =>
    ...
    linkedWithConditions = (engine: EngineAPI, startConditions: Conditions) => {
    ...
    matchType = (tfile: TFile, tags: string[], type: string): boolean => {
        ...
        switch (type) {
            case "area":
                return !!tags.find((t) => t?.startsWith("type/area"));
            case "encounter":
                // has  encounter attribute: new / active / completed, etc.
                return !!frontmatter.encounter;
            case "group":
                return !!tags.find((t) => t?.startsWith("type/group"));
            case "item":
                return !!tags.find((t) => t?.startsWith("type/item"));
            case "location":
                // include locations and areas
                return !!tags.find(
                    (t) =>
                        t &&
                        (t.startsWith("type/location") ||
                            t.startsWith("type/area")),
                );
            case "npc":
                return !!tags.find((t) => t?.startsWith("type/npc"));
            case "pc":
                return !!tags.find((t) => t?.startsWith("type/pc"));
        }
    ...
    npcTable = (
        ...
        const readNPC = (
            ...
            if (typeof value === "string") {
                ...
                for (const s of value) {
                    readNPC(tfile, s, iff, status, where, affiliation);
                }
            } else {
                addNPC(
                    tfile,
                    ...
                    (n) =>
                        `- <span class="npc">${
                            relationships
                                ...
                                : ""
                        }<span class="name">${n.link}</span><span class="affiliation">${n.affiliation.sort().join(", ")}</span><span class="where">${n.where}</span>${!pathPattern ? `<span class="scope">${n.scope}</span>` : ""}</span>`,
                )
                .join("\n"),
        );
    };
    ...
    placesTable = (
        ...
        for (const tfile of this.app.vault.getMarkdownFiles()) {
            ...
            const fileTags = this.utils().fileTags(tfile);
            const placeTag = fileTags.find((t) =>
                t?.startsWith("type/location"),
            );
            if (placeTag) {
                const affiliation: string[] = [];
    ...
    relatedNotItems = (engine: EngineAPI, tags: Tags = []): string => {
        ...
        const files = this.utils().filesMatchingCondition((tfile: TFile) => {
            ...
            const isType = fileTags.some((t) => t?.startsWith("type/"));
